...
class LeagueOfLegends(commands.Cog):
    ...
    @lol.sub_command(
        name="tierlist",
        description="Shows the tierlist of the line (top, mid, jungle, adc, support)",
    )
    async def tierlist_subcommand(
        self, interaction: disnake.ApplicationCommandInteraction, lane: LANES
    ):
        try:
            await interaction.response.defer(ephemeral=True)
            return await interaction.edit_original_message(
                content=f"⚠ This command is disabled for now, sorry for the inconvenience"
            )
            # data = None
            # # Create aiohttp session
            # session = ClientSession()
            # async with session.get(
            #     os.getenv("TIER_LIST_URI") + lane.lower()
            # ) as response:
            ...
            # if data is None:
            #     return await interaction.edit_original_message(
            #         content=f"⚠️ I have not been able to find any data"
            #     )

            # embed = disnake.Embed(
            #     colour=disnake.Colour.blurple(),
            #     title=f"{lane.capitalize() if not lane == 'ADC' else lane} Champion Tierlist",
            # )

            # for i, o in enumerate(data[:24]):
            #     embed.add_field(
            #         name=f"{CHAMPION_EMOJIS[o.get('name', 'None')]} {i + 1} - {o.get('name', 'None')}",
            #         value="\n".join(
            #             [
            #                 f"Ban Rate: {bold(o.get('ban_rate', '0.00%'))}",
            #                 f"Win Rate: {bold(o.get('win_rate', '0.00%'))}",
            #             ]
            #         ),
            #         inline=True,
            #     )
            ...
            print(f"❌: tierlist_subcommand_error: {str(_)}")
            return await interaction.edit_original_message(
                content=f"⚠️ I wasn't able to obtain the tierlist. I'm very sorry for this (If this error persists, please report it to the developers)"
            )

    @commands.cooldown(1, 20, commands.BucketType.user)
    @lol.sub_command(name="build", description="Shows the build of a champion")
    async def build_subcommand(
        self,
        interaction: disnake.ApplicationCommandInteraction,
        champion: str,
    ):
        ...
        return await interaction.edit_original_message(
            content=f"⚠️ This command is disabled for now, sorry for the inconvenience"
        )

        # if champion not in CHAMPIONS:
        #     return await interaction.edit_original_message(
        #         content=f"⚠️ That champion doesn't exist"
        #     )

        # try:
        #     c_ = (
        #         champion.replace("Master Yi", "masteryi")
        #         .replace("Nunu & Willump", "nunu")
        #         .replace("Kog'Maw", "kogmaw")
        #         .replace("Dr. Mundo", "drmundo")
        #         .replace("Kha'Zix", "khazix")
        #         .replace("Rek'Sai", "reksai")
        #         .replace("Aurelion Sol", "aurelionsol")
        #         .replace("Jarvan IV", "jarvaniv")
        #         .replace("Vel'Koz", "velkoz")
        #         .replace("Xin Zhao", "xinzhao")
        #         .replace("Lee Sin", "leesin")
        #         .replace("Miss Fortune", "missfortune")
        #     )
        #     c__ = (
        #         champion.replace(" ", "_")
        #         .replace("'", "")
        ...
        #     )
        #     file = disnake.File(
        #         fp=(await BROWSER.get_cached_screenshot(name=c_, action="build")),
        #         filename=f"Build_{c__}.png",
        #     )
        ...
        # except Exception as _:
        #     print(_)
        #     print(f"❌: build_subcommand_error: {str(_)}")
        #     return await interaction.edit_original_message(
        #         content=f"⚠️ I wasn't able to obtain the champion build. I'm very sorry for this (If this error persists, please report it to the developers)"
        #     )

    @build_subcommand.autocomplete("champion")
    async def build_autocomp(
        self, interaction: disnake.ApplicationCommandInteraction, input: str
    ) -> Dict[str, str]:
        string = input.lower()
        return [champ_name for champ_name in CHAMPIONS if string in champ_name.lower()][
    ...
    @commands.cooldown(1, 20, commands.BucketType.user)
    @lol.sub_command(name="skill", description="Shows the skill order of a champion")
    async def skill_subcommand(
        self, interaction: disnake.ApplicationCommandInteraction, champion: str
    ):
        await interaction.response.defer(ephemeral=True)
        ...
        #     )
        #     file = disnake.File(
        #         fp=(await BROWSER.get_cached_screenshot(name=c_, action="skills")),
        #         filename=f"Skill_{c__}.png",
        #     )
        #     return await interaction.edit_original_message(
        #         files=[file],
        #     )
        ...
        # except Exception as _:
        #     print(_)
        #     print(f"❌: skill_subcommand_error: {str(_)}")
        #     return await interaction.edit_original_message(
        #         content=f"⚠️ I wasn't able to obtain the skill order. I'm very sorry for this (If this error persists, please report it to the developers)"
        #     )

    @skill_subcommand.autocomplete("champion")
    async def skill_autocomp(
        self, interaction: disnake.ApplicationCommandInteraction, input: str
    ) -> Dict[str, str]:
        string = input.lower()
        return [champ_name for champ_name in CHAMPIONS if string in champ_name.lower()][
    ...
    @commands.cooldown(1, 20, commands.BucketType.user)
    @lol.sub_command(name="runes", description="Shows the runes of a champion")
    async def runes_subcommand(
        self, interaction: disnake.ApplicationCommandInteraction, champion: str
    ):
        await interaction.response.defer(ephemeral=True)
        ...
        try:
            ...
            file = disnake.File(
                fp=(await BROWSER.get_cached_screenshot(name=c_, action="runes")),
                filename=f"Runes_{c__}.png",
            )
            ...
            print(_)
            print(f"❌: runes_subcommand_error: {str(_)}")
            return await interaction.edit_original_message(
                content=f"⚠️ I wasn't able to obtain the runes. I'm very sorry for this (If this error persists, please report it to the developers)"
            )

    @runes_subcommand.autocomplete("champion")
    async def runes_autocomp(
        self, interaction: disnake.ApplicationCommandInteraction, input: str
    ) -> Dict[str, str]:
        string = input.lower()
        return [champ_name for champ_name in CHAMPIONS if string in champ_name.lower()][
    ...
    @lol.sub_command(
        name="stats",
        description="Get the stats of a champion",
    )
    async def stats_subcommand(
        self, interaction: disnake.ApplicationCommandInteraction, champion: str
    ):
        await interaction.response.defer(ephemeral=True)

        return await interaction.edit_original_message(
            content="⚠️ This command is disabled for now, sorry for the inconvenience"
        )
        ...
        #     )
        #     file = disnake.File(
        #         fp=(await BROWSER.get_cached_screenshot(name=c_, action="stats")),
        #         filename=f"Stats_{c__}.png",
        #     )
        #     return await interaction.edit_original_message(
        #         files=[file],
        #     )
        ...
        #     print(f"❌: skill_subcommand_error: {str(_)}")
        #     return await interaction.edit_original_message(
        #         content=f"⚠️ I wasn't able to obtain the champion information. I'm very sorry for this (If this error persists, please report it to the developers)"
        #     )

    @stats_subcommand.autocomplete("champion")
    async def stats_autocomp(
        self, interaction: disnake.ApplicationCommandInteraction, input: str
    ) -> Dict[str, str]:
        string = input.lower()
        return [champ_name for champ_name in CHAMPIONS if string in champ_name.lower()][
